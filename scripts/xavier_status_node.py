...
pub_cores = rospy.Publisher("/turquoise/xavier/cores_alive", Int16, queue_size=10)
...
pkg_path = rospack.get_path("xavier_stats")
for line in runProcess((pkg_path + '/scripts/tegrastats --interval ' + str(interval)).split()):
    if not rospy.is_shutdown():
        ...
        cores_alive = len(raw_data[5].split("off")) - 1
        ...
        pub_ram.publish(Float32(msg.ram))
        pub_cores.publish(Int16(cores_alive))
    else:
        break
